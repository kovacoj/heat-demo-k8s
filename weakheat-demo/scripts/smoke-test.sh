...
#   ./scripts/smoke-test.sh https://weakheat-kovacovsky-ns.dyn.cloud.e-infra.cz
set -euo pipefail

HOST=${1:?usage: smoke-test.sh https://host}
...
echo "== metrics"
curl -fsS "$HOST/api/metrics" | python3 -c '
import json, sys
m = json.load(sys.stdin)
print("OK: test median L2 %.2f%%" % (100 * m["test_relative_l2_median"]))
'
...
for i in $(seq 1 240); do
    ST=$(curl -fsS "$HOST/api/firedrake/$JOB_ID")
